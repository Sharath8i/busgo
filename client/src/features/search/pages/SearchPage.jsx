...
import { fetchBusesThunk } from '../searchSlice';
      ...
      <div className="mx-auto max-w-7xl px-6 py-8">
        <div className="grid gap-6 lg:grid-cols-[280px_1fr]">
          ...
          <div className="space-y-4">
            {isLoading ? (
              <Loading message="Fetching the best routes for you..." />
            ) : error ? (
              <EmptyState 
                icon="⚠️"
                title="Search Failed"
                ...
                onAction={() => dispatch(fetchBusesThunk({ from, to, date, seats }))}
              />
            ) : filtered.length === 0 ? (
              <EmptyState 
                title="No Buses Found"
                message={`We couldn't find any buses from ${from} to ${to} on ${date}. Try another date or route.`}
                actionLabel="Change Search"
                onAction={() => navigate('/')}
              />
            ) : (
              filtered.map(r => (
                <Card key={r.tripId} className="p-0 overflow-hidden hover:shadow-soft">
                  <div className="p-6">
                    <div className="flex flex-wrap items-start justify-between gap-6">
                      <div className="flex-1">
                        <div className="flex items-center gap-3 mb-1">
                          <h2 className="text-lg font-black text-text-main uppercase tracking-tight">{r.busName}</h2>
                          <Badge variant="primary">{r.busType?.replace('_', ' ')}</Badge>
                        </div>
